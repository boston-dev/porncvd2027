...
app.use(
  morgan(
    ":date[iso] :method :url :status :res[content-length] - :response-time ms rid=:req[x-request-id] ip=:req[x-forwarded-for]",
  ),
);
...
async function connectMongoWithRetry() {
  ...
  while (true) {
    ...
    try {
      ...
      console.error(
        `[mongo] connect failed (attempt ${attempt}): ${err.message}. retry in ${wait}ms`,
      );
